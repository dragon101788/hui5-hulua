...
HUI编程规范:
	一，所有代码高度抽象，整个HUI内核中，最形象的底线是控件，整个HUI内核中不允许出现业务逻辑相关任何代码
	二，规范化命名规则,	HUI<版本号> <子版本号>比如延伸版 HUI5.2.1 ugdraw
	三，从设计最初就是针对低端linux平台设计的UI式浏览器,与android有着截然不同的设计理念，不要盲目的跟风android的接口。
	四，所有接口规范化底层框架至少在大版本不做大的改动，以实现文档的通用性，让更多的人加入HUI，如果有改动框架需求，需取得每一个核心成员同意做出决定
	五，绘图逻辑一定要高度层抽象，设计绘图逻辑的时候一定要想象底层一片蓝天，中间鸟儿飞翔，在云中时隐时现的场面
	六，必须做到高度抽象化，模板化，禁止低效率，低可读，低容错的代码，所有代码中严禁嵌套超过两个以上的if else
	七，每个人固然有不同的思想，实现绘图逻辑另外的绘图逻辑要通过menuconfig 的方式，通过路由文件修改，要求在切换绘图逻辑时所有控件都可以编译使用，
		或者特殊控件kconfig 必须depend专用绘图逻辑
	八，添加任何新的控件kconfig必须给出help实例
	九，每一个版本都需要在此README中历史给出自己的设计思想，也就是说在写代码之前要先写出自己的需求与设计理念，再实现代码具体部分
	十，还有很多不完善的地方，希望大家能够积极提出，站在考虑HUI长远发展的角度

		抽象不应该向懒惰妥协，一个优秀的程序员不能向图方便妥协。
		我今天突然找出了10年写的HUI0.6，通过vc6编译器运行，一份优秀的代码不管多少年拿出来看的时候都能感觉那么舒服，有些接口可能忘掉了，
			但是程序依然能正常运转，依然能够正常的拿来开发。但是一份糟糕的代码到处飘着ifelse，多年之后你再想起它的时候只会让你感觉糟糕，头疼
													----胡鑫培 15/06/24
